...
class SeahawksThreadCog(commands.Cog):
    ...
    def fetch_schedule(self) -> list[dict[str, Any]]:
        """Return upcoming Seahawks games with opponent, short name, and start time (UTC)."""
            ...
            games.append(
                {
                    ...
                    "short_name": event.get("shortName", "SEA @ ???"),
                    "start": start,
                }
            )
        return games
    ...
    def _thread_title(self, short_name: str, start_pst: datetime) -> str:
        ts = start_pst.strftime("%-m/%-d, %-I:%M%p").lower()
        return f"\U0001F3C8 {short_name} ({ts} PST)"
    ...
    async def _open_threads(self) -> None:
        ...
        for g in games:
            ...
            if not isinstance(channel, discord.TextChannel):
                ...
                continue
            title = self._thread_title(g["short_name"], start_pst)
            try:
