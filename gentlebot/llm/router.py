...
class LLMRouter:
    def __init__(self) -> None:
        ...
        # Store images generated by tool calls for retrieval by calling code
        ...
        self.tool_limits = QuotaGuard(
            {
                ...
                "generate_image": Limit(rpm=5, rpd=100),
            }
        )
        self.base_dir = Path(os.getenv("GENTLEBOT_ROOT", Path.cwd())).resolve()

    def get_pending_images(self) -> list[bytes]:
        """Retrieve and clear pending images generated by tool calls.

        Returns:
            List of image bytes generated during tool execution.
        """
    ...
    def _run_search(self, params: dict[str, Any]) -> str:
        ...
        google_configured = bool(
            os.getenv("GOOGLE_SEARCH_API_KEY") and os.getenv("GOOGLE_SEARCH_CX")
        )
        if not google_configured:
            log.debug("tool=web_search status=no_google_credentials")
        ...
        if google_configured:
            try:
                ...
                failures.append(f"google_api:{type(e).__name__}")
                log.exception("tool=web_search status=google_error query=%s", query)
        else:
            failures.append("google_api:not_configured")

        # Fallback to Google via Jina
        if not results:
            try:
                results = _from_google_markdown()
            except Exception as e:
                failures.append(f"google_jina:{type(e).__name__}")
        ...
        if not results:
            try:
                ...
                failures.append(f"duckduckgo_api:{type(e).__name__}")
                log.exception("tool=web_search status=duckduckgo_error query=%s", query)

        # Final fallback to DuckDuckGo via Jina
        if not results:
            try:
                results = _from_duckduckgo_markdown()
            except Exception as e:
                failures.append(f"duckduckgo_jina:{type(e).__name__}")
                log.exception("tool=web_search status=duckduckgo_markdown_error query=%s", query)

        if not results:
            if failures:
                log.warning(
                    "tool=web_search status=all_backends_failed query=%s failures=%s",
                    query,
                    failures,
                )
                return f"Search failed. Tried backends: {', '.join(failures)}. Please try again later."
    ...
    def _run_generate_image(self, params: dict[str, Any]) -> str:
        """Generate an image from a text prompt and store it for retrieval."""
        ...
        try:
            ...
            return "Image generation is temporarily rate limited. Please try again later."
        except Exception as exc:
            log.exception("tool=generate_image status=error prompt=%s", prompt[:50])
            return f"Image generation failed: {exc}"

        if image_data:
            self._pending_images.append(image_data)
            # Truncate prompt in response for readability
            display_prompt = prompt[:100] + "..." if len(prompt) > 100 else prompt
            return f"[Image generated successfully for: {display_prompt}]"
        return "Image generation did not return data. The request may have been blocked."
